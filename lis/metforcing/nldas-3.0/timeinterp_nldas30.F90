...
!-----------------------------------------------------------------------
  do k = 1,LIS_rc%ntiles(n)/mfactor
     do m = 1,mfactor
        ...
        index1 = LIS_domain(n)%tile(t)%index
        kk = LIS_get_iteration_index(n,k,index1,mfactor)
     ! Total precip is a constant rate
...
!-----------------------------------------------------------------------
  do k = 1,LIS_rc%ntiles(n)/mfactor
     do m = 1,mfactor
        ...
        index1 = LIS_domain(n)%tile(t)%index
        kk = LIS_get_iteration_index(n,k,index1,mfactor)
        ! Longwave down shouldn't be interpolated between NLDAS-3 times - dmm
...
!-----------------------------------------------------------------------
  do k = 1,LIS_rc%ntiles(n)/mfactor
     do m = 1,mfactor
        ...
        tmp(t) = (nldas30_struc(n)%metdata1(kk,1,index1) * wt1) +      &
                 (nldas30_struc(n)%metdata2(kk,1,index1) * wt2)
        q2(t)  = (nldas30_struc(n)%metdata1(kk,2,index1) * wt1) +      &
                 (nldas30_struc(n)%metdata2(kk,2,index1) * wt2)
! Wind in NLDAS-3 is a time-averaged field and not interpolated - dmm
        uwind(t) = nldas30_struc(n)%metdata1(kk,5,index1)
        vwind(t) = nldas30_struc(n)%metdata1(kk,6,index1)
        psurf(t) = (nldas30_struc(n)%metdata1(kk,7,index1) * wt1) +    &
                   (nldas30_struc(n)%metdata2(kk,7,index1) * wt2)
     enddo
  enddo
