...
!  real      :: tair(nldas30_struc(n)%ncold,nldas30_struc(n)%nrold,24)
!  real      :: qair(nldas30_struc(n)%ncold,nldas30_struc(n)%nrold,24)
!  real      :: uwind(nldas30_struc(n)%ncold,nldas30_struc(n)%nrold,24)
!  real      :: vwind(nldas30_struc(n)%ncold,nldas30_struc(n)%nrold,24)
!  real      :: ps(LIS_rc%lnc(n),LIS_rc%lnr(n),24)
  real      :: ps(24)
!  real      :: rainf(nldas30_struc(n)%ncold,nldas30_struc(n)%nrold,24)
!  real      :: swdn(nldas30_struc(n)%ncold,nldas30_struc(n)%nrold,24)
!  real      :: lwdn(nldas30_struc(n)%ncold,nldas30_struc(n)%nrold,24)
...
!_______________________________________________________________________
  ...
  ferror = 0
  nr_index = nldas30_struc(n)%nrold
  nc_index = nldas30_struc(n)%ncold
...
! Read NLDAS-3 fields
  ...
  if (file_exists) then
     ...
     call LIS_verify(nf90_inq_varid(ftn,'PSurf',psId),                 &
          'nf90_inq_varid failed for ps in read_nldas30')
     call LIS_verify(nf90_get_var(ftn,psId,ps,start=(/6000,4000,1/),count=(/1,1,24/)),  &
          'nf90_get_var failed for ps in read_nldas30')
!     print *,LIS_rc%lnc(n),LIS_rc%lnr(n)
     print *,'PSurf = ',ps

     call LIS_endrun

!        call LIS_verify(nf90_inq_varid(ftn,'TLML',tmpId), &
!             'nf90_inq_varid failed for tlml in read_nldas30')
!        call LIS_verify(nf90_inq_varid(ftn,'QLML',qId), &
!             'nf90_inq_varid failed for qlml in read_nldas30')
!        call LIS_verify(nf90_inq_varid(ftn,'ULML',uId), &
!             'nf90_inq_varid failed for ulml in read_nldas30')
!        call LIS_verify(nf90_inq_varid(ftn,'VLML',vId), &
!             'nf90_inq_varid failed for vlml in read_nldas30')

!        call LIS_verify(nf90_get_var(ftn,tmpId, tair), &
!             'nf90_get_var failed for tlml in read_nldas30')
!        call LIS_verify(nf90_get_var(ftn,qId, qair), &
!             'nf90_get_var failed for qlml in read_nldas30')
!        call LIS_verify(nf90_get_var(ftn,uId, uwind), &
!             'nf90_get_var failed for ulml in read_nldas30')
!        call LIS_verify(nf90_get_var(ftn,vId, vwind), &
!             'nf90_get_var failed for vlml in read_nldas30')
!        call LIS_verify(nf90_close(ftn), &
!             'failed to close flxfile in read_nldas30')
...
!        call LIS_verify(nf90_inq_varid(ftn,'PRECTOTCORR',rainfId), &
!             'nf90_inq_varid failed for prectotcorr (flx) in read_nldas30')

!        call LIS_verify(nf90_get_var(ftn,rainfId,rainf), &
!             'nf90_get_var failed for prectotcorr (flx) in read_nldas30')

!     call interp_nldas30_var(n,findex,month,rainf,  8, .true.,nldasforc)

!     call LIS_verify(nf90_inq_varid(ftn,'SWDN',swdnId), &
!          'nf90_inq_varid failed for swdn in read_nldas30')
!     call LIS_verify(nf90_inq_varid(ftn,'LWDN',lwdnId), &
!          'nf90_inq_varid failed for lwdn in read_nldas30')

!     call LIS_verify(nf90_get_var(ftn,swdnId,swdn), &
!          'nf90_get_var failed for swgdn in read_nldas30')
!     call LIS_verify(nf90_get_var(ftn,lwdnId,lwdn), &
!          'nf90_get_var failed for lwgab in read_nldas30')
...
! !ARGUMENTS:
  ...
  integer, intent(in)    :: findex
  integer, intent(in)    :: month
  real,    intent(in)    :: input_var(nldas30_struc(n)%ncold,          &
                                      nldas30_struc(n)%nrold,24)
...
!EOP
  ...
  real      :: f (nldas30_struc(n)%ncold*nldas30_struc(n)%nrold)
  logical*1 :: lb(nldas30_struc(n)%ncold*nldas30_struc(n)%nrold)
  logical*1 :: lo(LIS_rc%lnc(n)*LIS_rc%lnr(n))
  integer   :: input_size
  logical   :: scal_read_flag
! _____________________________________________________________

  input_size = nldas30_struc(n)%ncold*nldas30_struc(n)%nrold

  do t = 1,24
     lb = .true.
     do r = 1,nldas30_struc(n)%nrold
        do c = 1,nldas30_struc(n)%ncold
           k = c+(r-1)*nldas30_struc(n)%ncold
           f(k) = input_var(c,r,t)
     ...
     if (pcp_flag.and.                                                 &
        ...
        call conserv_interp(LIS_rc%gridDesc(n,:),lb,f,lo,              &
             ...
             nldas30_struc(n)%mi,LIS_rc%lnc(n)*LIS_rc%lnr(n),          &
